...
class NetworkAllocationController extends ClientApiController
{
    ...
    public function store(NewAllocationRequest $request, Server $server): array
    {
        $allocation = Activity::event('server:allocation.create')->transaction(function ($log) use ($server) {
            // Only enforce limit if allocation_limit is explicitly set to a positive value.
            if ($server->allocation_limit > 0 && $server->allocations()->lockForUpdate()->count() >= $server->allocation_limit) {
