...
class MinecraftJavaLivePlayerProvider implements PlayerProviderInterface
{
     ...
     * @var array<int, array<string, string>>
     */
    private array $userCacheByNameCache = [];
    ...
    public function capabilities(Server $server): array
    {
        ...
        if (($status['sample_available'] ?? false) === false) {
            $notes[] = 'Server status sample is unavailable; only players with verified Mojang UUIDs recovered from server data can be displayed.';
        }
        $notes[] = 'Only players with a verified Mojang UUID/name match are displayed. Cracked or unverified identities are hidden.';
    ...
    private function status(Server $server): array
    {
        ...
        foreach ($hostCandidates as $host) {
            ...
            if ($onlineCount > 0) {
                // When the server hides status samples, reconstruct the online roster
                // from the current latest.log session. If the identity cannot be
                // confirmed, omit the player instead of emitting placeholders.
        ...
        if ($this->shouldUseConsoleRosterSnapshot($connected, $queryEnabled, $sampleAvailable, $onlineCount, $players)) {
            ...
            if ($consoleSnapshot !== null && !empty($consoleSnapshot['players'])) {
                $players = $consoleSnapshot['players'];
                $onlineCount = (int) $consoleSnapshot['online_count'];
                $sampleAvailable = true;
            }
        }
     ...
     */
    private function verifiedJavaIdentity(Server $server, string $name, string $uuid): ?array
    {
        $resolvedName = trim($name);
        if ($resolvedName === '' || $this->isGenericPlayerName($resolvedName)) {
            return null;
        }

        $candidateUuid = $this->normalizeUuid($uuid);
        if ($candidateUuid === '') {
            $candidateUuid = $this->normalizeUuid((string) ($this->userCacheByName($server)[mb_strtolower($resolvedName)] ?? ''));
        }
    ...
    /**
     * @param array<int, array{name: string, uuid: string, ping: int}> $players
     */
    private function shouldUseConsoleRosterSnapshot(
        bool $connected,
        bool $queryEnabled,
        ...
        if ($onlineCount !== count($players)) {
            return true;
        }

        foreach ($players as $player) {
            if ($this->normalizeUuid((string) ($player['uuid'] ?? '')) === '') {
                return true;
            }
        }
    ...
    /**
     * @return array{players: array<int, array{name: string, uuid: string, ping: int}>, online_count: int}|null
     */
    private function consoleRosterSnapshot(Server $server): ?array
    {
        $cacheKey = $this->consoleRosterSnapshotCacheKey($server);
        $cached = Cache::get($cacheKey);
        if (is_array($cached) && isset($cached['players'], $cached['online_count']) && is_array($cached['players'])) {
            return $cached;
        }

        $this->requestConsoleRosterSnapshot($server);
        usleep(800000);

        $snapshot = $this->latestUuidRosterSnapshotFromLog($server);
        if ($snapshot !== null) {
            Cache::put($cacheKey, $snapshot, now()->addSeconds(12));
        }

        return $snapshot;
    }

    private function consoleRosterSnapshotCacheKey(Server $server): string
    {
        return 'players:mcjava:console-roster:' . $server->id;
    }

    private function requestConsoleRosterSnapshot(Server $server): void
    {
        try {
            ...
            // Live command snapshots are best-effort only.
        }
    }
     ...
     * @return array{players: array<int, array{name: string, uuid: string, ping: int}>, online_count: int}|null
     */
    private function latestUuidRosterSnapshotFromLog(Server $server): ?array
    {
        $content = $this->readFirstAvailableTextFile($server, ['/logs/latest.log', '/latest.log'], 4 * 1024 * 1024);
        if ($content === '') {
            return null;
        }

        $lines = array_reverse(preg_split('/\r\n|\r|\n/', $content) ?: []);
        $countOnlySnapshot = null;
        ...
        foreach ($lines as $line) {
            ...
            $uuidSnapshot = $this->extractUuidRosterSnapshotFromListLine($normalizedLine);
            if ($uuidSnapshot !== null) {
                return $uuidSnapshot;
            }

            if ($countOnlySnapshot === null) {
                $countOnlySnapshot = $this->extractZeroPlayerSnapshotFromListLine($normalizedLine);
            }
        }

        return $countOnlySnapshot;
    }

    /**
     * @return array<int, array{name: string, uuid: string, ping: int}>
     */
    private function playersFromLatestLog(Server $server): array
    {
        $content = $this->readFirstAvailableTextFile($server, ['/logs/latest.log', '/latest.log'], 4 * 1024 * 1024);
        if ($content === '') {
            return [];
        ...
        $knownUuidsByName = $this->userCacheByName($server);
        $online = [];
        ...
        foreach ($lines as $line) {
            ...
            if ((bool) preg_match('/UUID of player ([A-Za-z0-9_]{1,16}) is ([0-9a-fA-F-]{32,36})/u', $normalizedLine, $matches)) {
                $nameKey = mb_strtolower($matches[1]);
                $uuid = $this->normalizeUuid($matches[2]);
                if ($uuid !== '') {
                    $knownUuidsByName[$nameKey] = $uuid;

                    if (isset($online[$nameKey])) {
                        $online[$nameKey]['uuid'] = $uuid;
                    }
                }
            ...
            $uuidListedPlayers = $this->extractOnlinePlayersFromUuidListLine($normalizedLine);
            if ($uuidListedPlayers !== null) {
                $online = $uuidListedPlayers;
                $hasAuthoritativeRoster = true;

                continue;
            }

            $listedPlayers = $this->extractOnlinePlayersFromListLine($normalizedLine, $knownUuidsByName);
            if ($listedPlayers !== null) {
                if (!$hasAuthoritativeRoster) {
                    $online = $listedPlayers;
                }

                continue;
            }

            $joinedName = $this->extractJoinedPlayerName($normalizedLine);
            if ($joinedName !== null) {
                $nameKey = mb_strtolower($joinedName);
                $online[$nameKey] = [
                    'name' => $joinedName,
                    'uuid' => $this->normalizeUuid((string) ($knownUuidsByName[$nameKey] ?? '')),
                    'ping' => 0,
                ];
            ...
            $leftName = $this->extractLeftPlayerName($normalizedLine);
            if ($leftName !== null) {
                unset($online[mb_strtolower($leftName)]);
            }
        }
    ...
    {
        $normalized = preg_replace('/\x1B\[[0-9;]*[A-Za-z]/', '', $line) ?? $line;
        $normalized = preg_replace('/§[0-9A-FK-ORX]/iu', '', $normalized) ?? $normalized;

        return trim($normalized);
    }

    /**
     * @param array<string, string> $knownUuidsByName
     * @return array<string, array{name: string, uuid: string, ping: int}>|null
     */
    private function extractOnlinePlayersFromUuidListLine(string $line): ?array
    {
        $snapshot = $this->extractUuidRosterSnapshotFromListLine($line);
        if ($snapshot === null) {
            return null;
        }
        ...
        foreach ($snapshot['players'] as $player) {
            ...
            $uuid = $this->normalizeUuid((string) ($player['uuid'] ?? ''));
            if ($name === '' || $uuid === '') {
                continue;
            }
     ...
     * @return array{players: array<int, array{name: string, uuid: string, ping: int}>, online_count: int}|null
     */
    private function extractUuidRosterSnapshotFromListLine(string $line): ?array
    {
        if (!(bool) preg_match('/There are\s+(\d+)\s+of a max of\s+\d+\s+players online:?\s*(.*)$/iu', $line, $matches)) {
            return null;
        }

        $onlineCount = max(0, (int) ($matches[1] ?? 0));
        ...
        preg_match_all('/([A-Za-z0-9_]{1,16})\s*\(([0-9a-fA-F-]{32,36})\)/u', $tail, $playerMatches, PREG_SET_ORDER);
        ...
        foreach ($playerMatches as $playerMatch) {
            $name = trim((string) ($playerMatch[1] ?? ''));
            $uuid = $this->normalizeUuid((string) ($playerMatch[2] ?? ''));
            if ($name === '' || $uuid === '') {
                continue;
            }
     ...
     * @return array{players: array<int, array{name: string, uuid: string, ping: int}>, online_count: int}|null
     */
    private function extractZeroPlayerSnapshotFromListLine(string $line): ?array
    {
        if (!(bool) preg_match('/There are\s+0\s+of a max of\s+\d+\s+players online:?\s*$/iu', $line)) {
            return null;
        }
     ...
     * @return array<string, array{name: string, uuid: string, ping: int}>|null
     */
    private function extractOnlinePlayersFromListLine(string $line, array $knownUuidsByName): ?array
    {
        if (!(bool) preg_match('/There are\s+\d+\s+of a max of\s+\d+\s+players online:?\s*(.*)$/iu', $line, $matches)) {
            return null;
        }
        ...
        $players = [];
        $names = preg_split('/\s*,\s*/', $tail) ?: [];

        foreach ($names as $name) {
            $trimmed = trim($name);
            if ($trimmed === '' || !(bool) preg_match('/^[A-Za-z0-9_]{1,16}$/u', $trimmed)) {
                continue;
            }

            $nameKey = mb_strtolower($trimmed);
            $players[$nameKey] = [
                'name' => $trimmed,
                'uuid' => $this->normalizeUuid((string) ($knownUuidsByName[$nameKey] ?? '')),
                'ping' => 0,
    ...
    private function extractJoinedPlayerName(string $line): ?string
    {
        $patterns = [
            '/\]:\s*([A-Za-z0-9_]{1,16})(?:\[[^\]]+\])?\s+joined the game\b/iu',
            '/\]:\s*([A-Za-z0-9_]{1,16})(?:\[[^\]]+\])?\s+logged in with entity id\b/iu',
        ];

        foreach ($patterns as $pattern) {
            if ((bool) preg_match($pattern, $line, $matches)) {
                return $matches[1];
            }
        }
    ...
    private function extractLeftPlayerName(string $line): ?string
    {
        $patterns = [
            '/\]:\s*([A-Za-z0-9_]{1,16})(?:\[[^\]]+\])?\s+left the game\b/iu',
            '/\]:\s*([A-Za-z0-9_]{1,16})(?:\[[^\]]+\])?\s+lost connection:/iu',
        ];
     ...
     * @return array<string, string>
     */
    private function userCacheByName(Server $server): array
    {
        if (isset($this->userCacheByNameCache[$server->id])) {
