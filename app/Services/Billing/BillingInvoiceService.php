    ...
    public function createNewOrderInvoice(User $user, array $data): BillingOrder
    {
        $order = DB::transaction(function () use ($user, $data) {
            ...
            $invoice = $this->createInvoiceRecord(
                ...
                profile: $profile,
                snapshot: $snapshot,
                provider: (string) config('billing.gateway.default', 'fiuu'),
                type: BillingInvoice::TYPE_NEW_SERVER,
    ...
    public function createRenewalInvoice(BillingSubscription $subscription, bool $notifyUser = false): BillingInvoice
    {
        ...
        $invoice = DB::transaction(function () use ($subscription) {
            ...
            $order = BillingOrder::query()->create([
                ...
                'split_limit' => $subscription->split_limit ?: ($subscription->server?->split_limit ?? $subscription->order?->split_limit ?? 0),
            ...
            $invoice = $this->createInvoiceRecord(
                ...
                profile: $profile,
                snapshot: $snapshot,
                provider: (string) ($subscription->gateway_provider ?: config('billing.gateway.default', 'fiuu')),
