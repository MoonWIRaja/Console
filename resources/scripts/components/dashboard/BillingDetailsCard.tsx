...
export default ({ cardClass, variant = 'card', onSaved }: Props) => {
    ...
    const formIsComplete = isBillingProfileComplete(form);
    const missingFieldLabels = getMissingBillingProfileLabels(form);
    ...
    const onSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setForm(updated);
            const updatedIsComplete = isBillingProfileComplete(updated);
            const updatedMissingLabels = getMissingBillingProfileLabels(updated);
            addFlash({
                key: 'account',
                type: 'success',
                title: 'Billing Details Saved',
                message: updatedIsComplete
                    ? 'Your billing details are complete and checkout can proceed.'
                    : `Billing details were saved, but checkout is still blocked until these fields are completed: ${updatedMissingLabels}.`,
            });
            onSaved?.();
        } catch (saveError) {
    ...
    const containerClassName = isDialog ? 'min-w-0 pt-1' : `${cardClass || ''} min-w-0 overflow-hidden`.trim();
                ...
                <div className={'mb-5 flex min-w-0 flex-wrap items-start justify-between gap-4'}>
                    <div className={'min-w-0 flex-1'}>
                        ...
                        <p className={'mt-1 break-words text-xs text-gray-400'}>
                            These details are printed on invoices and receipts. Checkout stays blocked until the
                ...
                <form onSubmit={onSubmit} className={'grid min-w-0 gap-5'}>
                    ...
                    <div className={'grid min-w-0 gap-4 lg:grid-cols-2'}>
                        ...
                        <div className={'min-w-0'}>
                            ...
                            <Input
                                value={form.taxId ?? ''}
                                onChange={(e) => onChange('taxId', e.currentTarget.value || null)}
                            />
                        </div>
                        <label
                            className={
                    ...
                    <div className={'flex justify-stretch border-t border-[color:var(--border)] pt-5 sm:justify-end'}>
                        <InteractiveHoverButton
                            ...
                            disabled={saving}
                            className={'w-full sm:w-auto !h-10 !min-w-0 sm:!min-w-[12rem] !px-5 !text-[10px]'}
                        />
                    </div>
                </form>
            )}
