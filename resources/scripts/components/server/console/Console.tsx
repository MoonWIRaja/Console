...
const shouldSuppressSystemConsoleLine = (line: string): boolean => {
    const normalized = normalizeConsoleLine(line);
    if (!normalized) {
        return false;
    }

    // Java: suppress "list uuids" command echo
    if (/\blist uuids\b/i.test(normalized)) {
        return true;
    }

    // Java: suppress "list uuids" response (header line with UUIDs in parentheses)
    if (
        /There are\s+\d+\s+of a max of\s+\d+\s+players online:/i.test(normalized) &&
    ...
    // Bedrock: suppress "There are X/Y players online:" header
    if (/There are\s+\d+\s*\/\s*\d+\s+players online:/i.test(normalized)) {
        return true;
    }
...
const isManualUuidRosterCommand = (command: string): boolean => {
    const normalized = command.trim().toLowerCase().replace(/\s+/g, ' ');

    return normalized === 'list uuids';
};

const isManualBedrockRosterCommand = (command: string): boolean => {
    const normalized = command.trim().toLowerCase().replace(/\s+/g, ' ');
...
export default ({
    ...
    const allowSystemConsoleOutputUntil = useRef(0);
    const suppressSystemBedrockListUntil = useRef(0);
    const awaitingSuppressedBedrockRosterNames = useRef(false);
    ...
    const handleConsoleOutput = useCallback(
        (line: string, prelude = false) => {
            ...
            // Always suppress system-generated roster output (Java + Bedrock)
            // unless the user recently typed the command manually.
            if (shouldSuppressSystemConsoleLine(line) && now > allowSystemConsoleOutputUntil.current) {
                // When suppressing a Bedrock header with no inline player names,
                // flag that the following line (the name list) should also be suppressed.
                const bedrockHeader = parseBedrockRosterHeader(normalizedLine);
                if (bedrockHeader && bedrockHeader.names.length === 0) {
                    awaitingSuppressedBedrockRosterNames.current = true;
                }
                return;
            }

            // Suppress the Bedrock player-name line that follows a suppressed header.
            if (awaitingSuppressedBedrockRosterNames.current && now > allowSystemConsoleOutputUntil.current) {
                const names = parseBedrockRosterNames(normalizedLine);
                if (names.length > 0) {
                    awaitingSuppressedBedrockRosterNames.current = false;
                    return;
                }
            ...
            pushTranscriptEntry(buildTranscriptEntry(line, { prelude }));
        },
    ...
    const handleCommandKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key === 'Enter' && command.length > 0) {
            if (isManualUuidRosterCommand(command) || isManualBedrockRosterCommand(command)) {
                allowSystemConsoleOutputUntil.current = Date.now() + 8000;
                awaitingSuppressedBedrockRosterNames.current = false;
            }
    ...
    useEffect(() => {
        const handleSystemBedrockList = (event: Event) => {
            const detail = (event as CustomEvent<{ serverId?: string }>).detail;
            if (!detail?.serverId || detail.serverId !== serverId) {
                return;
            }

            suppressSystemBedrockListUntil.current = Date.now() + 8000;
            awaitingSuppressedBedrockRosterNames.current = false;
        };

        window.addEventListener(BEDROCK_SYSTEM_LIST_EVENT, handleSystemBedrockList as EventListener);

        return () => {
            window.removeEventListener(BEDROCK_SYSTEM_LIST_EVENT, handleSystemBedrockList as EventListener);
        };
    }, [serverId]);
