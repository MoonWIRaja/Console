...
## v1.12.4
Upstream Pterodactyl `v1.12.2` → `v1.12.4` changes backported onto this fork, preserving all existing customizations.

### Security
* JWTs generated by the panel now carry a `scope` claim per subsystem (`websocket`, `file-upload`, `file-download`, `backup-download`, `transfer`), required by Wings `1.12.2+`. Resolves the `jwt: missing connect permission` websocket error after upgrading Wings.
* Adds a rate limit (3 changes / 24h) when changing account email addresses to prevent account enumeration.
* Backups, databases, and allocation creation now acquire a row lock before checking resource limits, closing a race-condition limit-bypass.
* API key creation is now wrapped in a locked transaction to prevent bypassing the per-account key limit.
* Remote transfer success/failure endpoints now authorize against the requesting node instead of the server's current node.

### Notes
* Wings on all nodes upgraded to `v1.12.3`.
* Intentionally not backported (would conflict with fork customizations or are not applicable): scheduled-task trait revert (fork has its own chaining fix), default-egg Java 25 data, Docker image/entrypoint changes, and test suite additions.
